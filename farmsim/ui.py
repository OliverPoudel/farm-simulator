...
def loadUI(screen):
    ...
    for i, item in enumerate(data.inventory):
        ...
        screen.blit(item_texture, (inventory_slots[i].x, inventory_slots[i].y))
        if item.type == "Crop":
            quantity_text = text_font.render(str(item.quantity), True, (255, 255, 255))
            screen.blit(quantity_text, (inventory_slots[i].right - quantity_text.get_width() - 5, inventory_slots[i].y + 5))